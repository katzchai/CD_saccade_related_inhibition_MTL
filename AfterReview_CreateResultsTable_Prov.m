...
% clear all
% 
% baseFolderPath = 'D:\Matlab\neurol\data\Saccade_New\Chaim_089_Demo\Saccade_ParsedData';% folder where the data  of that specific subject is located
% 
% cd (baseFolderPath)
...
% 
% %%%% %Get direction array 
% load('D:\Matlab\neurol\data\Saccade_New\Chaim_089_Demo\TWH089New.mat')%load direction vector (provided by Chaim)
% directionVector=directionVectorCleanedNew;%just naming the direction vector
% 
% 
% %%%%%%% %Change for every patient
% ResultsFolder='D:\Matlab\neurol\data\Saccade_New\RsultsSaccade_v3\TWH089';%Folder to save results
...
% TableFR=load('D:\Matlab\neurol\data\Saccade_New\RsultsSaccade_v2\TWH089\TWH089_FR_New.mat');%Table of Firing rates of randomized periods
...
% TableFR_IMG=load('D:\Matlab\neurol\data\Saccade_New\RsultsSaccade_v2\TWH089\TWH089_FR_New_IMG.mat');%Same as TableFR is a Table of randomized periods
% %%%%%%%%% %during the entire experiment. Consists of 1000 distributions of FR of 1500ms duration by the number of trials (40).
% load('D:\Matlab\neurol\data\Saccade_New\RsultsSaccade_v2\TWH089\corresp_electrode.mat');%This is a table I made
...
% TableSpikeTrains=load('D:\Matlab\neurol\data\Saccade_New\RsultsSaccade_v2\TWH089\Expanded_TWH089_Spike_epochs.mat');%This table has
% %%%%%%%%% %epoched data of spike trains. *Note 1.4 in word document
% SpikeWF=load('D:\Matlab\neurol\data\Saccade_New\Chaim_089_Demo\Saccade_ParsedData\spikeWF.mat');%this has information of the waveform per nueron
% LastTable=load('D:\Matlab\neurol\data\Saccade_New\RsultsSaccade_v2\TWH089\D_TWH089_ResultsTable_New.mat');

%% Take used neurons only

% FRGood=[LastTable.ResultsTable.FRGood];
% usedneurons= find(FRGood == 1);
% 
% 
% All= size(FRGood,2)
% included= size(usedneurons,2)
% 

 %%
NameResulttable="After_code_revisions"
%% Patient and Information %Change the selected fields accordingly
for ii= usedneurons(1:included);
    Neuron=ii;
   disp 'the neuron number is ' ;
   disp (Neuron);
   disp 'the Location is ' ;
   disp (LastTable.ResultsTable(Neuron).Location)
   disp 'the genneral effect is ' ; 
   disp (LastTable.ResultsTable(Neuron).TypeEffect)
   disp 'the imgOn effect is ' ;
   disp (LastTable.ResultsTable(Neuron).Final_ImgOn_effect)
...
%%
ResultsTable=AfterReview_MakeResultsTableNewProv(Px,ResultsFolder,NameResulttable,Location,Relevant_loc,SpikeWF,...
