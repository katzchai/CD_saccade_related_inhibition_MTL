...
FR_50perc_Img = mean(TableFR_IMG.averageFiringRatesImg(:,Neuron))/1.5
...
lineFR1=lineplot(FR_Min_Per, 'v', 'y', 'LineWidth', 2);

if  FR_Min_Per < percDown_Min 
   title ('Significant Decrease');
elseif  FR_Min_Per > percUP_Min
   title ('Significant Increase');
elseif FR_Min_Per >= percDown && mean_FR_Min_Per <= percUP_Min
   title ('No effect')
end
...
lineFR1=lineplot(FR_Max_Per, 'v', 'r', 'LineWidth', 2);

if  FR_Max_Per < percDown_Max 
   title ('Significant Decrease');
elseif  FR_Max_Per > percUP_Max
   title ('Significant Increase');
elseif FR_Max_Per >= percDown && FR_Max_Per <= percUP_Max
   title ('No effect')
end
...
N_all_FR_percent_ImgOn=(N_all_FR_ImgON-FR_50perc_Img)./abs(FR_50perc_Img).*100;%percent change  per neuron. These values were used later
%for the MUA latency modulation analysis. 
se_perc_ImgOn=(percUP_ImgOn-FR_50perc_Img)./abs(FR_50perc_Img).*100;
...
PromptGenAdj_IMG= 'Grade data confidence IMG data (1 to 5 for results) or 888 to abort   '
...
%% Make struct for analysis %Results where saved
 cd(ResultsFolder)
   ...
   % General firing rate
    ...
    ResultsTable(Neuron).FR_50perc_Img=FR_50perc_Img;
    ...
    ResultsTable(Neuron).distribution_bootstrap_control_IMG=distribution_bootstrap_controlIMG;
