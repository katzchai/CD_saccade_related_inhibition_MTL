...
if MacroERPPosNeg == 1
  ...
  %% RMS amplitude Note 
  resp_raw=mean_raw_LFPmacro(2510:2910);%We measured the RMS amplitude of the signal 10ms after saccade onset (to remove
  %artifact +500 ms
  resp_norm=mean_norm_LFPmacro(2510:2910);;
...
Pxfolder=[cell2mat(Px)];
 ...
 cd([Direction_ResultsFolder Pxfolder FolderNameDirection_LFP])
  ...
  resp_raw_Left=mean_raw_LFPmacro_Left(2510:2910);
  resp_norm_Left=mean_norm_LFPmacro_Left(2510:2910);
  
  rms_raw_Left=rms(resp_raw_Left);
  rms_norm_Left=rms(resp_norm_Left);

  resp_raw_Right=mean_raw_LFPmacro_Right(2510:2910);
  resp_norm_Right=mean_norm_LFPmacro_Right(2510:2910);
